...
这是一个**对照实验**，用来实证 `dev-notes/overview-20260924-0431.md` §3.3（“谁负责析构”
决定泄漏边界）的结论：一个"不在清盘时替对象负责"的 arena（这里用 `bumpalo::Bump`）会在若干
**普通写法**下真实泄漏对象持有的堆内存。
...
| 匿名临时值（`bump.alloc(..);`） | 66032 | 496 | **65536** | **0** | 泄漏 1 个 payload |
| 引用丢失（句柄离开作用域） | 66032 | 496 | **65536** | **0** | 泄漏 1 个 payload |
| 互相引用的环 | 131568 | 496 | **131072** | **0** | 泄漏 2 个 payload |
...
- sw-scope 的 `Retain` 是 GC Handle 式的长期句柄，允许对象先失去所有引用、再等 Scope
  ...
  路径——这就是 `dev-notes/overview-20260924-0431.md` §2.3 与 §3.3 那条"按类型去重的析构
  登记"存在的全部理由。
